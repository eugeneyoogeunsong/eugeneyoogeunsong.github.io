...
**What ten years of data say.** The full NOvA dataset - 384 $$\nu_\mu$$, 106 $$\bar\nu_\mu$$, 181 $$\nu_e$$ and 32 $$\bar\nu_e$$ candidates at the far detector - was published in January 2026 ([Phys. Rev. Lett. **136**, 011802](https://journals.aps.org/prl/abstract/10.1103/x53y-2b86), [arXiv:2509.04361](https://arxiv.org/abs/2509.04361)):

- $$\Delta m^{2}_{32} = 2.431^{+0.036}_{-0.034} \times 10^{-3}\ \mathrm{eV}^{2}$$ for the normal ordering, $$-2.479 \pm 0.036 \times 10^{-3}\ \mathrm{eV}^{2}$$ for the inverted - **the most precise single-experiment measurement of the atmospheric splitting there is**, at 1.4%.
- $$\sin^{2}\theta_{23} = 0.55^{+0.02}_{-0.06}$$: close to maximal, with only a mild upper-octant preference (69% with the Daya Bay constraint).
- A mild preference for the normal ordering, Bayes factor **2.4** - 70% of the posterior. Folding in Daya Bay's 2D $$\Delta m^{2}_{32}$$–$$\sin^{2}2\theta_{13}$$ constraint takes it to **6.6** (87%).
- The asymmetric combinations are the ones that fall: (IO, $$\delta_{CP} = \pi/2$$) is disfavoured at more than 4$$\sigma$$, (NO, $$3\pi/2$$) at about 2.5$$\sigma$$. Look at the bi-probability plot above and it is obvious why those two and not the others.

A Bayes factor is just the ratio of marginal likelihoods,
...
which is worth writing out because it makes the honest caveat visible: the integral runs over a prior, so the number quoted for the ordering is only as defensible as the prior on $$\delta_{CP}$$ underneath it. NOvA does not lean on a single fitter for this: the collaboration runs one frequentist fit and two independent Bayesian ones - a Metropolis-style MCMC and a Hamiltonian Monte Carlo implementation in Stan - and requires all three to agree before anything is published. **This is the part of the experiment I work on.**

**What has moved since.** At Neutrino 2026 in June the collaboration reported a set of stress tests on that dataset rather than new data, because the NuMI beam has been off since July 2024 after hitting a record 1 MW, and returns around September 2026 for a final run:

- Fitting the $$\nu_e$$-only and $$\nu_\mu$$-only subsamples **separately** gives consistent answers, and both are consistent with maximal mixing. The three-flavour paradigm survives being pulled apart.
- The first 3+1 sterile search to fit **both** beam modes at once - $$26.61 \times 10^{20}$$ protons on target in neutrino mode and $$12.50 \times 10^{20}$$ in antineutrino mode - finds no sterile neutrino and excludes most of the region IceCube still allows ([arXiv:2609.08900](https://arxiv.org/abs/2609.08900), September 2026; the neutrino-mode-only predecessor is [Phys. Rev. Lett. **134**, 081804](https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.134.081804)). The two ends of that limit are set by opposite things: at high $$\Delta m^{2}_{41}$$ the oscillations are fast and the sensitivity comes from the near detector's enormous statistics, so the limit is systematics-limited; at low $$\Delta m^{2}_{41}$$ they are slow, the far detector does the work, and the limit is statistics-limited. One analysis, two regimes, two entirely different things to worry about.
- Combining NOvA's $$\Delta m^{2}_{32}$$ with JUNO's takes the normal-ordering preference to a Bayes factor of about **16** (94% posterior, a 1.9$$\sigma$$ indication) - and, importantly, the $$\delta_{CP}$$ preference holds while the ordering preference moves, which is what a genuinely orthogonal constraint looks like ([arXiv:2606.14121](https://arxiv.org/abs/2606.14121)).
- The NOvA–T2K joint fit, meanwhile, leans weakly the other way ([Nature **646**, 818–824](https://www.nature.com/articles/s41586-025-09599-3), [arXiv:2510.19888](https://arxiv.org/abs/2510.19888)). Mass ordering and $$\delta_{CP}$$ stay degenerate even across two different baselines. Nobody is claiming this is settled.

NOvA remains **statistically limited**, not systematics-limited, which is an unusual and rather enviable position for a decade-old experiment: more data still buys more physics.
